...
sel.pattern(observations(oem)$ALB$idx[[1]])[, ac(2021:2045)] <-
  sel.pattern(observations(oem)$ALB$idx[[1]])[, ac(2020)]
...
# mp
projection(om) <- mseCtrl(method=fwdabc.om, args=list(pcbar=pcbar, pla=pla))
tes <- mp(om, oem=oem, ctrl=mpctrl, args=list(iy=2020, fy=2030))
...
# fwd @ catch=3500
ctrl <- fwdControl(year=2021:fy, quant="catch", value=23000)
